...
export const Onboarding: React.FC<Props> = ({ onComplete }) => {
  ...
  return (
    <div className="min-h-screen bg-stone-50 flex flex-col justify-center items-center px-4 py-8">
      <div className="w-full max-w-lg bg-white sm:shadow-xl sm:rounded-3xl p-6 sm:p-10 transition-all min-h-[500px] flex flex-col">
        
        {/* Progress Bar (Visible on steps 1-5) */}
        {step > 0 && step < 6 && (
          <div className="mb-8">
            <div className="flex justify-between text-xs text-stone-400 font-bold mb-2 uppercase tracking-wider">
        ...
        <div className="flex-1 flex flex-col justify-center">
          ...
          {step === 0 && (
            <div className="space-y-6 animate-fade-in text-center">
              ...
              <h1 className="text-3xl font-bold text-stone-800">Bem-vinda.</h1>
              <p className="text-lg text-stone-600">
                Este diário foi desenhado para entender você integralmente: corpo, mente e história.
              </p>
              <div className="pt-4 text-left">
                <label className="block">
                  <span className="text-stone-700 font-medium ml-1">Como você gostaria de ser chamada?</span>
                  <input 
                    type="text" 
                    className="mt-2 block w-full rounded-xl border-stone-300 bg-stone-50 p-4 shadow-sm focus:border-rose-500 focus:ring focus:ring-rose-200"
                    value={data.name}
          ...
          {step === 1 && (
            <div className="space-y-6 animate-fade-in">
              <div className="flex items-center gap-3 mb-2">
                <div className="p-2 bg-rose-100 rounded-lg text-rose-500"><Activity size={24} /></div>
                <h2 className="text-2xl font-bold text-stone-800">Biológico</h2>
              </div>
              
              <div className="grid grid-cols-2 gap-4">
                <label className="block">
                  <span className="text-stone-700 font-medium text-sm">Idade</span>
                  <input 
                    type="number" 
                    className="mt-1 block w-full rounded-xl border-stone-300 bg-stone-50 p-3"
                    value={data.age}
                    onChange={e => update('age', parseInt(e.target.value))}
                  />
                </label>
                <label className="block">
                  <span className="text-stone-700 font-medium text-sm">Última Menstruação</span>
                  <input 
                    type="date" 
                    className="mt-1 block w-full rounded-xl border-stone-300 bg-stone-50 p-3"
              ...
              <div>
                <span className="text-stone-700 font-medium block mb-2">Histórico Cirúrgico</span>
                <div className="space-y-2">
                  {[
                    ...
                    >
                      <div className={`w-5 h-5 rounded border flex items-center justify-center ${data.surgicalHistory.includes(opt.val) ? 'bg-rose-500 border-rose-500' : 'border-stone-300'}`}>
          ...
          {step === 2 && (
            <div className="space-y-6 animate-fade-in">
              <div className="flex items-center gap-3 mb-2">
                <div className="p-2 bg-teal-100 rounded-lg text-teal-500"><User size={24} /></div>
                <h2 className="text-2xl font-bold text-stone-800">Sociodemográfico</h2>
              </div>

              <label className="block">
                <span className="text-stone-700 font-medium">Estado Civil</span>
                <select 
                  className="mt-2 block w-full rounded-xl border-stone-300 bg-stone-50 p-3"
              ...
              <label className="block">
                <span className="text-stone-700 font-medium">Ocupação Principal</span>
                <input 
                  type="text" 
                  className="mt-2 block w-full rounded-xl border-stone-300 bg-stone-50 p-3"
          ...
          {step === 3 && (
            <div className="space-y-6 animate-fade-in">
              <div className="flex items-center gap-3 mb-2">
                <div className="p-2 bg-orange-100 rounded-lg text-orange-500"><Briefcase size={24} /></div>
                <h2 className="text-2xl font-bold text-stone-800">Contextual</h2>
              </div>

              <div className="block">
                <span className="text-stone-700 font-medium block mb-3">Você faz uso de Terapia Hormonal?</span>
                <div className="space-y-2">
                  {[
          ...
          {step === 4 && (
            <div className="space-y-6 animate-fade-in">
              <div className="flex items-center gap-3 mb-2">
                <div className="p-2 bg-purple-100 rounded-lg text-purple-500"><Calendar size={24} /></div>
                <h2 className="text-2xl font-bold text-stone-800">História de Vida</h2>
              </div>

              <div className="block">
                <span className="text-stone-700 font-medium block mb-3">Para você, a menopausa representa:</span>
                ...
                <textarea 
                   className="w-full rounded-xl border-stone-300 bg-stone-50 p-3 text-sm h-24"
          ...
          {step === 5 && (
            <div className="space-y-6 animate-fade-in">
              <div className="flex items-center gap-3 mb-2">
                <div className="p-2 bg-blue-100 rounded-lg text-blue-500"><Users size={24} /></div>
                <h2 className="text-2xl font-bold text-stone-800">Relações</h2>
              </div>

              <div>
                <span className="text-stone-700 font-medium block mb-3">Você conta com rede de apoio? (Família, amigos)</span>
              ...
              <label className="block">
                <span className="text-stone-700 font-medium block mb-2">Como você se sente com as mudanças no seu corpo?</span>
                <textarea 
                  className="w-full rounded-xl border-stone-300 bg-stone-50 p-3 text-sm h-24"
          ...
          {step === 6 && (
             ...
             <div className="w-20 h-20 bg-teal-100 rounded-full flex items-center justify-center mx-auto mb-4">
               <Smile className="text-teal-500 w-10 h-10" />
             </div>
             <h2 className="text-2xl font-bold text-stone-800">Perfil Completo!</h2>
             <p className="text-stone-600">
               Obrigada por compartilhar. Seus dados ajudarão a criar insights mais precisos para você.
             </p>
             
             <div className="bg-orange-50 p-4 rounded-xl text-orange-800 text-sm mt-4">
